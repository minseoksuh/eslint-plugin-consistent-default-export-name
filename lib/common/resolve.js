...
function tryRequire(target, sourceFile) {
    ...
    try {
        ...
        if (sourceFile != null) {
            try {
                resolved = createRequire(path.resolve(sourceFile)).resolve(
                    target
                );
            } catch (e) {
...
function fullResolve(modulePath, sourceFile, settings) {
    ...
    const cacheKey =
        sourceDir + hashObject(settings).digest('hex') + modulePath;
...
function requireResolver(name, sourceFile) {
    ...
    if (!isResolverValid(resolver)) {
        const err = new Error(
            `${name} with invalid interface loaded as resolver`
        );
        err.name = ERROR_NAME;
        throw err;
    }
